...
data.T.file <- c("groups_rect.csv","groups_rect.csv")
...
data.N.file <- c("gene_label.csv","gene_label.csv","","","","","","","","")
uploadtrack <- c(2,2,1,1,1,1,1,1,1,1)
...
tkrectcol <- 2
selrectcol <- 3
rectcols <- c("1:blue1","2:aquamarine")
...
data.TT <- data.T[[i]]
	...
	data.NN <- data.N[[i]]
	data.TT$num <- 1:nrow(data.TT)
tkbgcol <- c("deeppink3","deeppink3","deeppink3","deeppink3","deeppink3","deeppink3","deeppink3","deeppink3")
tkmargin <- 0.01
tkheight <- 0.09
tklinecoord <- c(0.25,0.75)
tklinecolor <- c("grey","grey")
